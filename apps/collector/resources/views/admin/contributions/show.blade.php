...
<form method="POST" action="{{ route('admin.contributions.validations.store',$contribution) }}">@csrf<div class="mb-3"><label class="form-label">Décision</label><select name="decision" class="form-select" required><option value="approve">Approuver</option><option value="correct">Corriger</option><option value="reject">Rejeter</option></select></div><div class="mb-3"><label class="form-label">Variété validée</label><select name="variety_id" class="form-select"><option value="">Non applicable / rejet</option>@foreach($varieties as $variety)<option value="{{ $variety->id }}">{{ $variety->name }}{{ $variety->iso_code ? ' ('.$variety->iso_code.')' : '' }}</option>@endforeach</select></div><div class="mb-3"><label class="form-label">Texte corrigé (si décision = Corriger)</label><textarea name="san_text_corrected" class="form-control" rows="4"></textarea></div><div class="mb-3"><label class="form-label">Notes</label><textarea name="notes" class="form-control" rows="3"></textarea></div><button class="btn btn-success"><i class="bi bi-check2-circle"></i> Valider</button></form></div></div></div>
</div></section>
@endsection
